...
void generate_sub_keys(unsigned char* main_key, key_set* key_sets) {
	int i = 0, j = 0;
	int shift_size = 0;
	unsigned char shift_byte = 0, first_shift_bits = 0, second_shift_bits = 0, third_shift_bits = 0, fourth_shift_bits = 0;
...
void process_message(unsigned char* message_piece, unsigned char* processed_piece, key_set* key_sets, int mode) {
  ...
  inline
int DES_ECB_PKCS5Padding(char *out,char *in,int file_size,char* des_key,char process_mode)
{
	unsigned short int padding = 0;
    unsigned long block_count = 0, number_of_blocks = 0;
	int processed_len = 0;
	unsigned char data_block[8] = {0};
	unsigned char processed_block[8] = {0};
	unsigned char key_sets[272] = {0};

	generate_sub_keys(des_key, (key_set *)key_sets);
	...
	while(1)
	{
		if((block_count + 1) != number_of_blocks)
		{
			memcpy(data_block,in + block_count*8,8);
			process_message(data_block, processed_block, (key_set *)key_sets, process_mode);
		...
		else
		{
			if(process_mode == ENCRYPTION_MODE)
			{
				...
				{
					memset(data_block, (unsigned char)padding,8);
					memcpy(data_block,in + block_count*8,8 - padding);
					process_message(data_block, processed_block, (key_set *)key_sets, process_mode);
					processed_len = processed_len + 8;
					memcpy(out + block_count*8,processed_block,8);
					goto success;
			...
			else
			{
					memcpy(data_block,in + block_count*8,8);
					process_message(data_block, processed_block, (key_set *)key_sets, process_mode);
					padding = processed_block[6];
					if (padding < 8) 
					{
						processed_len = processed_len + 8 - padding;
						memcpy(out + block_count*8,processed_block,8 - padding);
					}
					goto success;	
			}
		}
